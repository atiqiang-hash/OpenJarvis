...
h('0. 我的持仓（My Portfolio · 置顶）', 2)
para('页面顶部新增的个人持仓面板，数据来自你的券商（Midas）截图：')
bullet('四张汇总卡：总资产（≈USD）、美股 ABD、土耳其股 BIST、现金，均含当日盈亏。')
bullet('一条占比色条 + 图例，显示各标的在总资产中的权重。')
bullet('持仓明细表：标的 / 市场 / 持股 / 现价 / 日涨跌 / 持仓市值 / ≈USD / 占比 / 数据来源。')
para('美股（QQQ、VOO）走 Finnhub 实时（标「实时」）；土耳其 BIST 三只（BIMAS、ASELS、ASTOR）'
     '免费层无实时，按截图快照显示（标「截图」）。要更新持仓：用文本编辑器打开 index.html，'
     '搜索 const PORTFOLIO，按格式修改持股、价格、当日涨跌；汇率改 FX_TRY_USD。', color=DIM, size=9.5)

h('0b. 资产配置蓝图（$3,000 怎么分）', 2)
para('把账户分成三桶：核心稳健（宽基 ETF 打底）、卫星成长（质优成长 / Serenity A 档）、'
     '投机高波动（卡点小盘 / BIST / 加密）。提供「保守 75/20/5」「稳健 70/20/10」「进取 50/30/20」三档预设，'
     '自动算出每桶金额，并给出关键纪律：单只投机仓 ≤ 账户 5%、投机总仓有上限。')

h('0c. 仓位与风险计算器（最重要）', 2)
para('买之前先算清楚风险。输入账户规模、单笔风险%（默认 1%）、入场价、止损价、目标价，立即得出：')
bullet('单笔最大风险（如 $3,000 的 1% = $30）')
bullet('建议买入股数 = 最大风险 ÷ 每股风险')
bullet('仓位价值与占账户比例、触发止损的实际亏损')
bullet('盈亏比 R:R，以及 1R/2R/3R 分批止盈参考价')
para('铁律：盈亏比 < 2:1 的交易尽量别做；止损设好后只能上移、不能下挪。这是小账户活下来的第一要素。',
     italic=True, color=DIM, size=9.5)

h('0d. 交易日志与绩效', 2)
para('记录每一笔交易（标的 / 方向 / 股数 / 入场 / 出场 / 日期 / 备注），自动统计：累计已实现盈亏、'
     '胜率、盈亏比（均盈/均亏）、每笔期望值。出场价留空 = 持仓中。数据保存在你本机浏览器（localStorage），'
     '可「载入示例」预览、「清空日志」重来。期望值 > 0，才是长期能赚钱的系统。')
para('注意：这些工具帮你控制风险、保持纪律、复盘提升，但不保证盈利——盈亏取决于市场和你自己的决策。',
     bold=True, color=DIM, size=9.5)

h('0e. 复利 vs 回撤模拟器', 2)
para('输入本金、月均收益%、月数、以及中途一次回撤%，画出「纪律线 vs 踩雷线」两条净值曲线，'
     '直观看清一次大亏的代价，并给出回撤恢复数学（亏 X% 要赚 X/(1-X) 才回本）。'
     '例：$3,000 每月 +3% 滚 24 个月 ≈ $6,098；若中途一次 -50%，终值只剩 ≈ $3,049。核心结论：保住本金 > 追求暴利。')

h('0f. 价格预警', 2)
para('为盯住的标的设「≥ 目标价 / ≤ 止损价」。每次刷新行情时自动检查，触发即高亮；'
     '点「🔔 桌面通知」授权后，触发会弹系统通知。预警保存在本机浏览器，仅美股有实时价。')

h('0g. 持仓诊断（一键体检）', 2)
para('把「我的持仓」自动分桶（核心 / 卫星 / 投机），对照你选的配置蓝图，逐条给出诊断：'
     '哪只投机仓超过 5% 单仓上限、投机总仓是否超配、成长仓是否不足等。随实时行情自动刷新结论。')
para('止损闭环：在「我的持仓」表的「止损价」列直接给每个持仓填止损（保存在本机浏览器），'
     '右侧「风险敞口」列实时显示一旦触发止损会亏多少（金额与占账户%）。诊断会自动区分核心 ETF'
     '（长期持有可不设硬止损，建议挂 -20% 灾难止损）与卫星/投机仓（必须设止损），并标出缺止损、'
     '止损过宽（单笔 > 账户 1%）、止损无效（≥ 现价）等问题。', color=DIM, size=9.5)
